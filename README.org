...
Copy and paste the following script into the macro editor (/Plugins/ → /New/ → /Macro/). Edit the input and output paths (look for the lines below the =FIXME= comments).
...
for (i = 0; i < titles.length; i++) {
    if (titles[i].contains("Cellpose-SAM")) {
        ...
        saveAs("Tiff", "/absolute/output/path/Cellpose-SAM_segments.tif");
        break;
    }
}
#+end_src
...
fiji --headless -macro /path/to/cellpose_sam.ijm
#+end_src
