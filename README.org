...
for (i = 0; i < files.length; i++) {
    // The files list contains all files in the directory, including
    // other directories, so check if the file name ends in ".tif" or
    // ".tiff" and starts with "t". This pattern is highly specific to
...
// Concatenate all open files. This will create a new time axis,
// resulting in a 4-dimensional image.
...
* Inspect results
Look through the z-stack of cell segments for a specific time point and compare it to the corresponding microscopy image. Do you notice anything that looks like oversegmentation or undersegmentation? If so, what parameter(s) could you change in Cellpose-SAM to improve this?
